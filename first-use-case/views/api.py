...
from models.api_models import BundleHashModel
from models.api_models import AddressModel
from models.api_models import TagModel
from models.api_models import TransactionModel
from models.api_models import AddressTokenReceivedModel
...
from models.api_models import DateModel
...
def getAddressInfo(addressInput):
    ...
    try:
        ...
        #in case of multiple objects it will through MultipleObjectsReturned Excep$
        isSpentFrom = True
    ...
    if isSpentFrom == True:
        ...
        #If the address is not spent_from then further check if it is received onl$
        #If it has received any token then it means is received only.
        #Otherwise its a no value transfer and value will be equal to 0 in that ca
        ...
        try:
            ...
            # in case of multiple objects it will through MultipleObjectsReturned $
            isReceivedOnly = True
...
@api.route("/tag-search/<string:tag_input>")
...
def get_distinct_addresses():
    ...
    return jsonify({"total_distinct_addresses": result.count()} )
